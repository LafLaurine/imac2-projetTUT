...
class FeatureWarper:
    ...
    def warp_person(self, person: det.Person, list_array_features):
        # TODO: discard faces which do not display the necessary points of interest?
        i = 0
        for face in person:
            self.warp_face(face, list_array_features[i])
            i += 1

    def warp_face(self, face: det.Face, array_features):
        """
        the three points of interest are:
        the eyes and the mouth
        """
        # first, we compute the positions of the three points of interest
        points_interest_face = FeatureWarper.__get_points_interest(array_features)
        ...
        face.replace_image(image_warped)
    ...
    def __get_position_left_eye(array_features):
        return FeatureWarper.__get_position_landmark_average(array_features, "left_eye")
    @staticmethod
    def __get_position_right_eye(array_freatures):
        return FeatureWarper.__get_position_landmark_average(array_freatures, "right_eye")
    @staticmethod
